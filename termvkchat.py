        ...
        def print_list_dialogs():
            ...
            for index, dialog in enumerate(self.results['items']):
                ...
                if online:
                    print(' [online]', end='')
                if unread > 0:
                    print(' (unread: {})'.format(unread), end='')
                print()
...
class ChatDraw:
    ...
    def loadHistory(self, results, mark=[]):
        ...
        for i in range(h, l):
            ...
            self.pc.add_message_top(sender, post, time=time, mark=mark, prnt=False)
